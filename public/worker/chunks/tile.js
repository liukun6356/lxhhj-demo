define(['exports', '/arcgis-js-api/geometry/Extent.js', './polygon'], (function (exports, Extent, polygon) { 'use strict';
  ...
  function createNamespaceError(namespace) {
      return (e) => new Error(`${namespace} ${e}`);
  }
  exports.ColorMappingTypeCode = void 0;
  (function (ColorMappingTypeCode) {
      ColorMappingTypeCode[ColorMappingTypeCode["gradient"] = 1] = "gradient";
      ColorMappingTypeCode[ColorMappingTypeCode["class-break"] = 2] = "class-break";
      ColorMappingTypeCode[ColorMappingTypeCode["unique-value"] = 3] = "unique-value";
  })(exports.ColorMappingTypeCode || (exports.ColorMappingTypeCode = {}));
  var RenderSamplingCode;
  (function (RenderSamplingCode) {
      RenderSamplingCode[RenderSamplingCode["nearest"] = 1] = "nearest";
      RenderSamplingCode[RenderSamplingCode["linear"] = 2] = "linear";
  })(RenderSamplingCode || (RenderSamplingCode = {}));
  //查找后续的动画帧对应时间的索引
  function getFramePrediction(opts) {
      const { times, curTime } = opts;
      let frame;
      frame = Math.max(opts.preload?.frame ?? 5, 2);
      const [beforeIndex, afterIndex] = polygon.findIntervalIndexThatValueIn(times, curTime);
      const indexMax = times.length - 1;
      const timeIndexSet = new Set();
      for (let i = beforeIndex, count = frame + 1; //默认向后取帧
       count > 0 && i <= indexMax; count--, i++) {
          timeIndexSet.add(i);
      }
      return { timeIndexSet, beforeIndex, afterIndex };
  }
  const ILayerIdKey = Symbol("internal_layer_uid");
  const getLayerUID = /*#__PURE__*/ (() => {
      let id = 1;
      return () => id++;
  })();
  function checkTimes(ascTimes) {
      const length = ascTimes?.length;
      if (!length)
          throw new Error("时间序列长度不能为空");
      ...
      for (let i = 0; i < length; i++) {
          ...
          if (i && curTime < ascTimes[i - 1])
              throw new Error(`时间序列必须是严格递增的`);
          if (indexMap.has(curTime))
              throw new Error(`存在重复的时间${curTime}`);
  ...
  (function (EnumTaskStatus) {
      EnumTaskStatus["none"] = "none";
      EnumTaskStatus["pending"] = "pending";
      EnumTaskStatus["finish"] = "finish";
      EnumTaskStatus["error"] = "error";
  })(EnumTaskStatus || (EnumTaskStatus = {}));
  //异步任务调度器, 只负责调度
  function asyncTaskScheduler(opts) {
      let autoExcuteNext = true;
      let toDoQueue = []; //优先级， 后->先
      ...
      function execute() {
          if (runningMap.size === maxConcurrency)
              return;
      ...
      function cancelTask(key, reason) {
      ...
      //返回数组优先级[低 -> 高]
      function updateQueue(filterFn) {
          const newTasks = filterFn.call(null, Array.from(runningMap.values()).map((i) => ({ desc: i.desc, cancel: i.cancel })), [...toDoQueue]);
  ...
  function idleSyncTaskScheduler(opts) {
      let todo = [];
      const todoKeySet = new Set();
      let _timer;
      ...
      const start = () => {
          ...
          if (!todo.length)
              return;
          _timer = requestIdleCallback(function step(deadline) {
              try {
                  const state = {};
  ...
  function resolveViewRenderExtent(view) {
      if (!view.rotation)
          return view.extent.clone();
  ...
  function isRectIntersect(rect1, rect2) {
      return !((rect1.xmin + rect1.width) < rect2.xmin
  ...
  (function (RegionIndex) {
      RegionIndex[RegionIndex["topLeft"] = 3] = "topLeft";
      RegionIndex[RegionIndex["topRight"] = 2] = "topRight";
      RegionIndex[RegionIndex["bottomLeft"] = 0] = "bottomLeft";
      RegionIndex[RegionIndex["bottomRight"] = 1] = "bottomRight";
  ...
  class Quadtree {
       ...
       */
      constructor(bounds, maxObjects, maxLevels, level, region) {
       ...
       */
      split() {
          const nextLevel = this.level + 1, { xmin, ymin, width, height, cx, cy } = this.bounds, halfWidth = width / 2, halfHeight = height / 2;
      ...
      getIndex(pRect) {
          ...
          if (width === 0 && height === 0) { //点
              ...
              if (x < cx) {
                  ...
                  if (y < cy)
                      return [RegionIndex.bottomLeft, RegionIndex.bottomRight];
                  if (y > cy)
                      return [RegionIndex.topLeft, RegionIndex.topRight];
                  return [
              ...
              const indexes = [];
              const isStartLeft = width ? xmin < cx : xmin <= cx, isEndRight = xmin + width > cx, isStartBottom = height ? ymin < cy : ymin <= cy, isEndTop = ymin + height > cy;
              if (isStartLeft && isStartBottom)
                  indexes.push(RegionIndex.bottomLeft);
              if (isEndRight && isStartBottom)
                  indexes.push(RegionIndex.bottomRight);
              if (isEndRight && isEndTop)
                  indexes.push(RegionIndex.topRight);
              if (isStartLeft && isEndTop)
                  indexes.push(RegionIndex.topLeft);
              return indexes;
          }
      }
      ;
       ...
       */
      retrieve(pRect) {
          let targetIndices = this.getIndex(pRect), returnObjects = [];
          if (this.subTrees.length) {
              const childRetrieves = [];
          ...
          function indicesIntersects(arr1, arr2) {
  ...
  function createVTile(sourceFeatures, tileInfo, tolerance, sourceExtent) {
      let tileNumPoints = 0, tileNumSimplify = 0;
      const tileFeatures = []; //after simplify
      ...
      function addSimplifiedFeature(srcVFeature, tolerance) {
          const { type } = srcVFeature;
          const common = {
              id: srcVFeature.id,
              attributes: srcVFeature.attributes,
          };
          let featureNum = 0, featureSimplify = 0;
          if (type === "point") {
              tileNumPoints += 1;
              tileNumSimplify += 1;
      ...
      function simplifyPath(path, tolerance) {
          let numPoints = 0, numSimplified = 0, containDis = !!path.distance, disResult = containDis ? [] : null, result = [];
          ...
          const p0 = result[0], pn = result[result.length - 1];
      ...
      function simplifyPolygonRing(ring, tolerance) {
          let numPoints = 0, numSimplified = 0, result = [];
      ...
      function resolveExtent(extent, points) {
  ...
  function getVTileInfo(...args) {
      let z, x, y, origin, resolution, scale, tileSize;
      if (typeof args[3] === "number") {
          [z, x, y, resolution, scale, origin, tileSize] = args;
      }
      else {
          [z, x, y] = args;
          const tileScheme = args[3];
          const lod = tileScheme.lods.find((i) => i.level === z);
          if (!lod)
              throw new Error("找不到对应等级");
   ...
   * @return [tl, tr, bl, br]
   */
  function getChildrenTiles(z, x, y) {
      z = z + 1;
      x = x * 2;
      y = y * 2;
      return [
  ...
  function findCoverTileBoundsAtGivenLevel(extent, tileScheme, level) {
  ...
  function getBoundsExtent(level, bounds, tileScheme) {
      const size = tileScheme.size[0];
      const origin = tileScheme.origin;
      const resolution = tileScheme.lods.find((i) => i.level === level).resolution;
  ...
  function tileSpiralGenerator(bounds, gen, reverse = true) {
  ...
  function findCoverVTilesAtGivenLevel(extent, tileScheme, level) {
      const result = [];
  ...
  function findLodAtGivenResolution(resolution, tileScheme) {
      const index = tileScheme.lods.findIndex((i) => {
          return i.resolution < resolution;
      });
      if (index === -1)
          return tileScheme.lods.slice(-1)[0];
      return tileScheme.lods[Math.max(0, index - 1)];
  }
  //创建带缓存的vtile getter
  function createVTileTreeWithScheme(opts) {
      ...
      function parentGetter() {
          const { z, x, y } = this;
          if (z <= minLevel)
              return null;
          if (!this._parent) {
              const { resolution, scale } = lodMap[z - 1];
              const parent = wrap(getVTileInfo(z - 1, Math.floor(x / 2), Math.floor(y / 2), resolution, scale, origin, tileSize));
              this._parent = parent;
  ...
  exports.checkTimes = checkTimes;
  exports.clamp = clamp;
  exports.createNamespaceError = createNamespaceError;
  exports.createVTile = createVTile;
  exports.findCoverTileBoundsAtGivenLevel = findCoverTileBoundsAtGivenLevel;
  exports.findCoverVTilesAtGivenLevel = findCoverVTilesAtGivenLevel;
  exports.findLodAtGivenResolution = findLodAtGivenResolution;
  exports.getChildrenTiles = getChildrenTiles;
  exports.getTileId = getTileId;
  exports.getVTileInfo = getVTileInfo;
  exports.isAbortError = isAbortError;
  exports.isNil = isNil;
  exports.tileIdToZXY = tileIdToZXY;
