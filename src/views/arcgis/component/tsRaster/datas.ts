...
import gradientImg from "./color-step.png";
    ...
    return {
        ...
        source: {
            ...
            dataGetter: async (time: number) => {
                const fileBuffer = await fetch(`/test-data/timing-raster/clud/YR_lucc_${time}.tif`).then((res) =>
                    res.arrayBuffer()
                );
...
export function getDataDam(): TsRasterDataMeta {
    ...
    return {
        ...
        source: {
            ...
            crs: { wkid: 4549 },
            dataGetter: async (time: number) => {
                const res = await fetch(`/test-data/timing-raster/溃坝/hs_${time}.00sec.bin`);
...
export function getDataDamVectorField(): TsRasterDataMeta {
    ...
    return {
        ...
        source: {
            ...
            crs: { wkid: 4549 },
            dataGetter: async (time: number) => {
                const fileBuffer = await fetch(`/test-data/timing-raster/vector-field/v_${time}.00sec.tif`).then(
                    (res) => res.arrayBuffer()
                );
...
export function getDataLake(): TsRasterDataMeta {
    return {
        ...
        source: {
            ...
            crs: { wkid: 2436 },
            dataGetter: async (time: number) => {
                const res = await fetch(`/test-data/timing-raster/COD/COD_${("000" + time).slice(-3)}.bin`);
                const result = await res.arrayBuffer();
                return new Float32Array(result);
            },
        },
...
export function getDataUIS(): TsRasterDataMeta {
    return {
        ...
        source: {
            ...
            crs: { wkid: 54009 },
            dataGetter: async (time: number) => {
                const fileBuffer = await fetch(`/test-data/timing-raster/uis/YR_UIS_${time}.tif`).then((res) =>
                    res.arrayBuffer()
                );
