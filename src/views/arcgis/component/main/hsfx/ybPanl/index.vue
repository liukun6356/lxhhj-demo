      ...
      <div class="right-area">
        ...
        <div class="time-player">
          ...
          <el-slider v-model="curTime" :disabled="playFlag"
                     ...
                     :step="5*60*1e3" style="width:720px;margin-left: 10px"/>
          <div class="tip-area">
          ...
          <el-select class="tip-area-select" v-model="speedType" size="small" style="width: 40px" :disabled="playFlag"
...
interface Props {
  ...
  class?: string // 类目
  timeType?: "h" | "m" | "5m" | "s" // 需反应时间点类型,小时级|分钟级|秒级
...
watch(() => model.curTime, (v) => {
  ...
  // if (model.curTime % getTimeType(props.timeType) === 0)
    emits("timeChange", model.curTime)
})
...
const togglePlay = () => {
  ...
  if (model.playFlag) {
    ...
    // }, 1000 / model.speedType / 60 * 2)

    timer = requestAnimationFrame(function step() {
      const forward = model.speedType;
      const nextTime = curTime.value + forward;
      model.curTime = model.curTime + 16.67 * 1e3 * model.speedType;
      console.log(model.curTime,123)
      if (nextTime >= model.sliderTimeRang[1]) {
        model.playFlag = false;
...
const submit = () => {
  ...
  model.sliderTimeRang = [+model.formData.startTime, +model.formData.startTime + (model.formData.range - 1) * 60 * 60 * 1e3]
