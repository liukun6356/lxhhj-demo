...
class CustomLayerView2D extends BaseLayerViewGL2D {
    ...
    attach() {
        ...
        ctx.fillStyle = "skyblue"; // 字体颜色
        ctx.shadowColor = "red";
        ctx.strokeStyle = "white"; // 网格线 grid颜色
        ctx.lineWidth = 1;
        ctx.shadowBlur = 0;
    ...
    manageTextures() {
        ...
        for (let i = 0; i < this.tiles.length; ++i) {
            ...
            if (!this.textures.has(tile.id)) {
                ...
                this.ctx.fillText([tile.level, tile.col, tile.row, tile.world].join("/"), 10, 24);
    ...
    render(renderParameters: BaseLayerViewGL2DRenderRenderParameters) {
        this.manageTextures();
        ...
        gl.bindBuffer(gl.ARRAY_BUFFER, this.vertexBuffer); // [0, 0, 1, 0, 1, 1, 0, 1]
        gl.vertexAttribPointer(this.a_Position, 2, gl.UNSIGNED_BYTE, false, 2, 0);
        gl.bindBuffer(gl.ELEMENT_ARRAY_BUFFER, this.indexBuffer);
        gl.uniformMatrix3fv(this.u_NDCTransform, false, this.ndcTransform); // 投影矩阵
        gl.uniformMatrix3fv(this.u_ModelTransform, false, this.modelTransform); // 正常矩阵
