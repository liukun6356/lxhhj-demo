...
<template>
  <teleport to="body">
    <ul v-show="showPopup" :class="['surveyStation-popup','map'+mapStore.mapType]" ref="popupRef"
      ...
      <li>名称：{{ info.name }}</li>
      <li>雨量：--mm</li>
      <li>库前水位：{{ info.reservoirWaterLevel ? info.reservoirWaterLevel : '--' }} m</li>
    </ul>
  </teleport>
  <el-dialog v-model="dialogVisible" :show-close="false" class="globalDetailDialogClass" append-to-body
             destroy-on-close>
    <reservoir-station-dialog :id="curId" :listData="listData" @icon-close="dialogVisible=false"/>
  </el-dialog>
</template>
...
// Component
import ReservoirStationDialog from "./reservoirStationDialog/index.vue"
...
onMounted(() => {
  getlist()
  reservoirStationDatasource = new Cesium.CustomDataSource("reservoirStation");
  viewer.dataSources.add(reservoirStationDatasource);
  ...
  mittBus.on("skipReservoirStationDialog", mittBusFn)
})

onUnmounted(() => {
  handler.destroy()
  mittBus.off("skipReservoirStationDialog", mittBusFn)
  reservoirStationDatasource?.entities?.removeAll()
  viewer.scene.postRender.removeEventListener(showPopupBox);
  viewer.dataSources.remove(reservoirStationDatasource);
})

watch(() => mapStore.mapType, () => {
  reservoirStationDatasource.entities._entities._array.forEach(async (entity) => {
    ...
    entity.billboard.image = iconTypes["reservoir"][mapStore.mapType][1]
  })
})

const mittBusFn = (row) => {
  const {id} = row
  const curEntity = reservoirStationDatasource.entities.values.find(entity => entity.id === id)
...
const getlist = async () => {
  setTimeout(() => {
    model.listData = mapStore.allPointPosList["reservoirStationEntity"]
...
let reservoirStationDatasource, PreSelEntity, handler
const addEntity = (data) => {
  data.forEach(async (item) => {
    reservoirStationDatasource.entities.add({
      customType: "reservoirStationEntity",
      ...
      billboard: {
        image: iconTypes["reservoir"][mapStore.mapType][1],
...
const onMouseMove = (movement) => {
  ...
  if (!(entity instanceof Cesium.Entity) || entity.customType !== "reservoirStationEntity") return
