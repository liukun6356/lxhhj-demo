...
<template>
  <div class="controlPanel-wrap">
    ...
    <div class="rightTool">
      ...
      <!-- 反遮罩-->
      <Boundary v-if="boundaryShow && terrainProviderShow"/>
      <!-- 概化图-->
      <Teleport to="body">
  ...
  switch (type) {
    ...
    case "3d":
      if (!viewer.terrainProvider.isFlag && mapStore.curSelectTool !== "数值") {
        const terrainProvider = await Cesium.CesiumTerrainProvider.fromUrl(import.meta.env.VITE_APP_GISDATA + "/cz/dem/chenzhouDem")
        model.terrainProviderShow = true
        //     await mars3d.LayerUtil.createTerrainProvider({
