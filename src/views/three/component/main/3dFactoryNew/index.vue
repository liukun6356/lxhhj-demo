...
const {listData, info, showPopup, popupPos} = toRefs(model)
onMounted(() => {
  getlist()
  init()
  initminiMap()
...
const init = () => {
  ...
  outlinePass.visibleEdgeColor.set('#000'); // 颜色
  outlinePass.edgeStrength = 2.0;// 亮度
  outlinePass.edgeThickness = 3; // 描边厚度
  outlinePass.pulsePeriod = 0; // 闪烁频率，每 1 秒闪烁一次
  outlinePass.hiddenEdgeColor.set('#fff') //边线颜色
...
const onMouseClick = (e) => {
  ...
  while (mesh.isChild) {//gltf的hover只显示边线
    mesh = mesh.parent
  }
...
const createBasePanel = () => {
  ...
  // 设置网格的位置
  gridPanel.position.set(0, -1, 0); // 这里的坐标根据你的需求来设置
  group.add(gridPanel);
...
const createGradientPlane = () => {
  ...
  const height = 12 * 5;
  const geometry = new three.PlaneGeometry(width, height); // 创建平面几何体
  // 顶点着色器代码
  ...
  const mesh = new three.Mesh(geometry, material); // 创建平面模型
...
const addIndicatorBar = (data, position) => {
  ...
  const mesh = new THREE.Mesh(geometry, material);
  mesh.rotateX(-Math.PI / 2); // 默认平行于xoy面，沿着X轴旋转-90°至xoz面
...
const addImgBubble = async (text, position, floorPanelMesh) => {
  if (!text) return
  const texture = await textureLoader.loadAsync(yellowCirSvg)// yellowCirSvg// redCirSvg
  ...
  const canvasTexture = new THREE.CanvasTexture(canvas) // 转成纹理
  const spriteMaterial = new THREE.SpriteMaterial({
    map: canvasTexture,
  });
  const sprite = new THREE.Sprite(spriteMaterial);
  sprite.position.set(position.x, position.y, position.z + 2); //设置位置
  sprite.scale.set(2.5, 2.5, 2.5); //设置精灵大小
  sprite.center.set(0.5, 0); //设置位置点处于精灵的最下方中间位置
  sprite.customType = "yellow-bubble"
  sprite.visible = false
  floorPanelMesh.add(sprite)

  const texture2 = await textureLoader.loadAsync(redCirSvg)// yellowCirSvg// redCirSvg
  ...
  const canvasTexture2 = new THREE.CanvasTexture(canvas2) // 转成纹理
  const spriteMaterial2 = new THREE.SpriteMaterial({
    map: canvasTexture2,
  });
  const sprite2 = new THREE.Sprite(spriteMaterial2);
  sprite2.position.set(position.x, position.y, position.z); //设置位置
  sprite2.scale.set(2, 2, 2); //设置精灵大小
  sprite2.center.set(0.5, 0); //设置位置点处于精灵的最下方中间位置
